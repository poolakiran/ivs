       ...
       struct l3_cidr_route_entry *route_entry, uint32_t size)
{
    ...
    lpm64_trie_insert(lpm64_trie, route_entry->key, route_entry->mask_len,
                      &route_entry->value);
    assert(lpm64_trie->size == size);
}
...
search(char *key, uint32_t *expected_value)
{
    ...
    uint32_t *value = lpm64_trie_search(lpm64_trie, ipv6_to_key(key));
...
delete(char *key, uint8_t mask_len, uint32_t size)
{
    ...
    lpm64_trie_remove(lpm64_trie, ipv6_to_key(key), mask_len);
    assert(lpm64_trie->size == size);
}
...
test_basic(void)
{
...
#ifndef DEBUG
    print_preorder(lpm64_trie->root);
#endif
    ...
    assert(lpm64_trie->size == 0);
    lpm64_trie_destroy(lpm64_trie);
}
...
test_random()
{
    ...
    for (i = 0; i < NUM_ENTRIES; i++) {
        ...
        lpm64_trie_insert(lpm64_trie, key, mask_len, &route_entries[i].value);
    }
    ...
    for (i = 0; i < num_lookups; i++) {
        ...
        uint32_t *lpm64_value = lpm64_trie_search(lpm64_trie, key);

        /* Lookup in the route_entries array for lpm associated with the key */
        uint32_t *ref_value = linear_search(key);

#ifndef DEBUG
        printf("lpm for ipv6: %s, lpm64_value: %u, ref_value: %u\n", key_to_ipv6(key),
               lpm64_value? *(uint32_t *)lpm64_value: 0,
               ref_value? *(uint32_t *)ref_value: 0);
#endif

        AIM_ASSERT(lpm64_value == ref_value, "mismatch with reference");
    }

    /* Remove entries */
    for (i = 0; i < NUM_ENTRIES; i++) {
        lpm64_trie_remove(lpm64_trie, route_entries[i].key, route_entries[i].mask_len);
        route_entries[i].valid = false;
    }

    /* Verify that the trie is empty after all the operations */
    assert(lpm64_trie_is_empty(lpm64_trie) == true);
    lpm64_trie_destroy(lpm64_trie);
}
...
test_mixed()
{
    ...
    for (i = 0; i < num_lookups; i++) {
        ...
        if (route_entries[index].valid == false &&
            ...
            lpm64_trie_insert(lpm64_trie, key, mask_len, &route_entries[index].value);
        } else {
            lpm64_trie_remove(lpm64_trie, route_entries[index].key,
    ...
    for (i = 0; i < NUM_ENTRIES; i++) {
        if (route_entries[i].valid == true) {
            lpm64_trie_remove(lpm64_trie, route_entries[i].key,
...
test_churn()
{
    ...
    for (int i = 0; i < 100; i++) {
        for (uint64_t j = 0; j < 32; j++) {
            lpm64_trie_insert(lpm64_trie, j<<16, 48, (void *)1);
        }
        for (uint64_t j = 0; j < 32; j++) {
            lpm64_trie_remove(lpm64_trie, j<<16, 48);
        }
    }
