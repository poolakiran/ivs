...
static inline struct lpm64_trie_entry *
lpm64_trie_entry_object(struct lpm64_trie *lpm64_trie, uint32_t slot)
{
    AIM_ASSERT(slot != SLOT_INVALID, "invalid slot");

    return &((lpm64_trie)->lpm64_trie_entries[slot]);
}
...
/*
 * Create a lpm64 trie entry node
 */
static uint32_t
trie_entry_create(struct lpm64_trie *lpm64_trie, uint64_t key, uint8_t mask_len,
                  uint8_t bit, void *value, uint32_t left, uint32_t right)
{
    AIM_LOG_TRACE("Create lpm64 trie entry with ipv6 key=%"PRIx64"/%u, bit=%u",
                  key, mask_len, bit);

    uint32_t slot = slot_allocator_alloc(lpm64_trie->lpm64_trie_entry_allocator);
    AIM_ASSERT(slot != SLOT_INVALID, "Failed to create slot for lpm64 trie entry");

    if (slot >= lpm64_trie->allocated) {
        AIM_LOG_TRACE("Growing lpm64 trie to %d entries", lpm64_trie->allocated * 2);
        lpm64_trie->allocated = roundup(slot*2);
        AIM_ASSERT(lpm64_trie->allocated > slot, "LPM64 did not grow to contain new slot");
        lpm64_trie->lpm64_trie_entries =
            aim_realloc(lpm64_trie->lpm64_trie_entries, lpm64_trie->allocated * sizeof(struct lpm64_trie_entry));
    }

    struct lpm64_trie_entry *entry = lpm64_trie_entry_object(lpm64_trie, slot);
...
/*
 * Remove a lpm64 trie entry node
 */
static void
trie_entry_remove(struct lpm64_trie *lpm64_trie, struct lpm64_trie_entry *current,
                  struct lpm64_trie_entry *parent, uint32_t current_slot)
{
    /*
     ...
     */
    struct lpm64_trie_entry *current_child = NULL;
    uint32_t current_child_slot = SLOT_INVALID;
    if (current->left != SLOT_INVALID) {
        current_child = lpm64_trie_entry_object(lpm64_trie, current->left);
        current_child_slot = current->left;
    } else if (current->right != SLOT_INVALID) {
        current_child = lpm64_trie_entry_object(lpm64_trie, current->right);
     ...
     * becomes null
     */
    if (lpm64_trie->root == current_slot) {
        lpm64_trie->root = current_child_slot;
        slot_allocator_free(lpm64_trie->lpm64_trie_entry_allocator, current_slot);
        return;
    }
     ...
     */
    struct lpm64_trie_entry *sibling;
    uint32_t sibling_slot;
    if (is_current_left_child) {
        sibling = lpm64_trie_entry_object(lpm64_trie, parent->right);
        sibling_slot = parent->right;
    } else {
        sibling = lpm64_trie_entry_object(lpm64_trie, parent->left);
    ...
    slot_allocator_free(lpm64_trie->lpm64_trie_entry_allocator, sibling_slot);
    slot_allocator_free(lpm64_trie->lpm64_trie_entry_allocator, current_slot);
    return;
}

/*
 * Documented in lpm64.h
 */
struct lpm64_trie *
lpm64_trie_create(void)
{
    struct lpm64_trie *lpm64_trie = aim_zmalloc(sizeof(struct lpm64_trie));

    lpm64_trie->lpm64_trie_entry_allocator = slot_allocator_create(LPM64_TRIE_ENTRY_COUNT);
    lpm64_trie->lpm64_trie_entries = aim_malloc(LPM64_TRIE_INITIAL_ALLOCATION *
                                            sizeof(struct lpm64_trie_entry));
    lpm64_trie->root = SLOT_INVALID;
    lpm64_trie->size = 0;
    lpm64_trie->allocated = LPM64_TRIE_INITIAL_ALLOCATION;
 ...
 */
void
lpm64_trie_destroy(struct lpm64_trie *lpm64_trie)
{
    AIM_ASSERT(lpm64_trie != NULL, "attempted to delete a NULL lpm64 trie");
    AIM_ASSERT(lpm64_trie->root == SLOT_INVALID, "attempted to delete a non empty lpm64 trie");

    slot_allocator_destroy(lpm64_trie->lpm64_trie_entry_allocator);
    aim_free(lpm64_trie->lpm64_trie_entries);
    aim_free(lpm64_trie);
}
 ...
 */
bool
lpm64_trie_is_empty(struct lpm64_trie *lpm64_trie)
{
    AIM_ASSERT(lpm64_trie != NULL, "attempted to determine the size of a NULL lpm64 trie");

    return (lpm64_trie->size == 0)? true : false;
}
 ...
 */
int
lpm64_trie_insert(struct lpm64_trie *lpm64_trie, uint64_t key,
                uint8_t key_mask_len, void *value)
{
    AIM_ASSERT(lpm64_trie != NULL, "attempted to insert a entry in a NULL lpm64 trie");
    ...
    AIM_LOG_TRACE("Add lpm64 trie entry with ipv6 key=%"PRIx64"/%u", key, key_mask_len);

    if (lpm64_trie->root == SLOT_INVALID) {
        /* First entry */
        lpm64_trie->root = trie_entry_create(lpm64_trie, key, key_mask_len,
     ...
     */
    uint32_t current_idx = lpm64_trie->root;
    struct lpm64_trie_entry *current = lpm64_trie_entry_object(lpm64_trie, current_idx);
    ...
    while (true) {
        while (index < current->match_bit_count) {
            ...
            if ((total_index + index) >= key_mask_len) {
                 ...
                 */
                uint32_t entry_slot = trie_entry_create(lpm64_trie, current->key,
                                                        current->mask_len,
                                                        current->match_bit_count-index,
                                                        current->value, current->left,
                                                        current->right);
                current = lpm64_trie_entry_object(lpm64_trie, current_idx);
            ...
            if (current_bit != key_bit) {
                 ...
                 */
                uint32_t new_entry_slot = trie_entry_create(lpm64_trie, key,
                                                            key_mask_len,
                                                            key_mask_len-total_index-index,
                                                            value, SLOT_INVALID, SLOT_INVALID);
                current = lpm64_trie_entry_object(lpm64_trie, current_idx);

                uint32_t old_entry_slot = trie_entry_create(lpm64_trie,
                                                            current->key,
                                                            current->mask_len,
                                                            current->match_bit_count-index,
                                                            current->value, current->left,
        ...
        if ((total_index + index) < key_mask_len) {
            ...
            if (is_bit_set(key, total_index + index)) {
                /* left branch */
                if (current->left == SLOT_INVALID) {
                    uint32_t entry_slot = trie_entry_create(lpm64_trie, key,
                                                            key_mask_len,
                                                            key_mask_len-total_index-index,
                                                            value, SLOT_INVALID,
                                                            SLOT_INVALID);
 ...
 */
void *
lpm64_trie_search(struct lpm64_trie *lpm64_trie, uint64_t key)
{
    AIM_ASSERT(lpm64_trie != NULL, "attempted to search for a entry in a NULL lpm64 trie");

    void *result_value = NULL;

    uint32_t current_slot = lpm64_trie->root;

    int total_index = 0;

    AIM_LOG_TRACE("Search lpm64 trie for key=%"PRIx64, key);

    while (current_slot != SLOT_INVALID) {
        struct lpm64_trie_entry *current = lpm64_trie_entry_object(lpm64_trie, current_slot);
        ...
        if (current->value != NULL) {
            AIM_LOG_TRACE("Found lpm64 trie entry with ipv6=%"PRIx64"/%u for "
 ...
 */
void
lpm64_trie_remove(struct lpm64_trie *lpm64_trie, uint64_t key, uint8_t key_mask_len)
{
    AIM_ASSERT(lpm64_trie != NULL, "attempted to remove a entry in a NULL lpm64 trie");

    struct lpm64_trie_entry *parent = NULL;
    uint32_t current_slot = lpm64_trie->root;

    int total_index = 0;

    AIM_LOG_TRACE("Remove lpm64 trie entry with ipv6 key=%"PRIx64"/%u", key, key_mask_len);
    ...
    while (current_slot != SLOT_INVALID) {
        ...
        if ((key_mask_len - total_index) < current->match_bit_count) {
            AIM_LOG_TRACE("No lpm64 trie entry present with ipv4=%"PRIx64"/%u",
                          key, key_mask_len);
            return;
        }
        ...
        if (total_index == key_mask_len) {
            /* Found the node */
            if (current->value != NULL) {
                trie_entry_remove(lpm64_trie, current, parent, current_slot);
                lpm64_trie->size -= 1;
            }
            return;
        }
