    ...
    indigo_cookie_t flow_id, void **entry_priv)
{
    ...
    AIM_LOG_VERBOSE("Entry group=%u vlan=%u eth-src=%{mac} eth-dst=%{mac} cpu=%d drop=%d",
                    entry->value.next_hop.lag ? entry->value.next_hop.lag->id : OF_GROUP_ANY, entry->value.next_hop.new_vlan_vid, &entry->value.next_hop.new_eth_src, &entry->value.next_hop.new_eth_dst, entry->value.cpu, entry->value.drop);
