...
parse_value(of_flow_add_t *obj, struct ingress_mirror_value *value)
{
    ...
    if (group_to_table_id(value->span_id) != GROUP_TABLE_ID_SPAN) {
        AIM_LOG_WARN("Unexpected group table ID in ingress_mirror table");
        return INDIGO_ERROR_COMPAT;
    }
    ...
    indigo_cookie_t flow_id, void **entry_priv)
{
    ...
    AIM_LOG_VERBOSE("Create ingress_mirror entry in_port=%u -> span_id %u",
                    entry->key.in_port, entry->value.span_id);
...
{
    ingress_mirror_hashtable = bighash_table_create(64);
...
pipeline_bvs_table_ingress_mirror_lookup(const struct ingress_mirror_key *key)
{
