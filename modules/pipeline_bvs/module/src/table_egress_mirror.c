    ...
    OF_LIST_INSTRUCTION_ITER(&insts, &inst, rv) {
        ...
        case OF_INSTRUCTION_APPLY_ACTIONS: {
            ...
            OF_LIST_ACTION_ITER(&actions, &act, rv) {
                ...
                case OF_ACTION_BSN_GENTABLE: {
                    if (!seen_span_id) {
                        ...
                        if (table_id == pipeline_bvs_table_span_id) {
                            ...
                            AIM_LOG_ERROR("unsupported gentable reference in egress_mirror table");
                            goto error;
                        }
                    } else {
                        AIM_LOG_ERROR("Duplicate SPAN action in egress_mirror table");
                ...
                default:
                    AIM_LOG_ERROR("Unexpected action %s in egress_mirror table", of_object_id_str[act.object_id]);
                    goto error;
                }
            }
            break;
        ...
        default:
            AIM_LOG_ERROR("Unexpected instruction %s in egress_mirror table", of_object_id_str[inst.object_id]);
            goto error;
        }
    }
...
pipeline_bvs_table_egress_mirror_lookup(uint32_t port_no)
{
    ...
    if (entry) {
        packet_trace("Hit egress_mirror entry out_port=%u -> span_id %u",
                     entry->key.out_port, entry->value.span->id);
    } else {
        packet_trace("Miss egress_mirror entry out_port=%u", key.out_port);
    }
    return entry;
}
