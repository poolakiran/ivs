...
parse_value(of_flow_add_t *obj, struct source_miss_override_value *value)
{
    ...
    OF_LIST_INSTRUCTION_ITER(&insts, &inst, rv) {
        ...
        case OF_INSTRUCTION_APPLY_ACTIONS: {
            ...
            OF_LIST_ACTION_ITER(&actions, &act, rv) {
                ...
                case OF_ACTION_OUTPUT: {
                    of_port_no_t port;
                    of_action_output_port_get(&act.output, &port);
                ...
                default:
                    AIM_LOG_ERROR("Unexpected action %s in source_miss_override table", of_object_id_str[act.header.object_id]);
                    goto error;
                }
            }
            break;
        ...
        default:
            AIM_LOG_ERROR("Unexpected instruction %s in source_miss_override table", of_object_id_str[inst.header.object_id]);
            goto error;
        }
    }
