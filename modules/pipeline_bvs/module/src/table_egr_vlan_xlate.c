...
static const of_match_fields_t vlan_xlate_port_group_id_mask = {
    .bsn_vlan_xlate_port_group_id = 0xffffffff,
    .vlan_vid = 0xffff,
};
static const of_match_fields_t in_port_mask = {
    .in_port = 0xffffffff,
    .vlan_vid = 0xffff,
};
...
parse_key(of_flow_add_t *obj, struct egr_vlan_xlate_key *key)
{
    ...
    if (!memcmp(&match.masks, &vlan_xlate_port_group_id_mask, sizeof(of_match_fields_t))) {
        key->vlan_xlate_port_group_id = match.fields.bsn_vlan_xlate_port_group_id;
        key->port = 0;
        key->vlan_vid = match.fields.vlan_vid & ~VLAN_CFI_BIT;
        key->pad = 0;
    } else if (!memcmp(&match.masks, &in_port_mask, sizeof(of_match_fields_t))) {
        key->vlan_xlate_port_group_id = 0;
...
pipeline_bvs_table_egr_vlan_xlate_lookup(uint32_t vlan_xlate_port_group_id, uint32_t port, uint16_t vlan_vid)
{
    ...
    if (entry) {
        packet_trace("Hit egr_vlan_xlate entry vlan_xlate_port_group_id=%u, port=%u, vlan=%u -> vlan %u",
                     entry->key.vlan_xlate_port_group_id, entry->key.port, entry->key.vlan_vid,
                     entry->value.new_vlan_vid);
    } else {
        packet_trace("Miss egr_vlan_xlate entry vlan_xlate_port_group_id=%u, port=%u, vlan=%u",
                     key.vlan_xlate_port_group_id, key.port, key.vlan_vid);
    }
    return entry;
}
