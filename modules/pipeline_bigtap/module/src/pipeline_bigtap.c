...
static void pipeline_bigtap_update_cfr(struct ind_ovs_cfr *cfr, struct xbuf *actions);
...
{
    if (!strcmp(name, "bigtap-1.0")) {
        openflow_version = OF_VERSION_1_0;
    } else if (!strcmp(name, "bigtap-1.3")) {
                          ...
                          struct pipeline_result *result)
{
    ...
    while (table_id != (uint8_t)-1) {
        ...
        if (table_id != (uint8_t)-1) {
            pipeline_bigtap_update_cfr(cfr, &effects->apply_actions);
        }
    }
...
{
    pipeline_register("bigtap-1.0", &pipeline_bigtap_ops);
    pipeline_register("bigtap-1.3", &pipeline_bigtap_ops);
}
 ...
 */
static void
pipeline_bigtap_update_cfr(struct ind_ovs_cfr *cfr, struct xbuf *actions)
{
    struct nlattr *attr;
