...
parser = argparse.ArgumentParser(description='Barrier benchmark')
...
request_buf = ofp.message.barrier_request(xid=0).pack()
request_chunk = request_buf * chunk_size
reply_buf = ofp.message.barrier_reply(xid=0).pack()
...
print "%d barrier messages in %f.1s (%.0f barrier/s)" % (count, elapsed, count/elapsed)
