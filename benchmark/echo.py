...
parser = argparse.ArgumentParser(description='Echo benchmark')
...
request_buf = ofp.message.echo_request(xid=0).pack()
request_chunk = request_buf * chunk_size
reply_buf = ofp.message.echo_reply(xid=0).pack()
...
print "%d echo messages in %f.1s (%.0f echo/s)" % (count, elapsed, count/elapsed)
