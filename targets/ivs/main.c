...
parse_options(int argc, char **argv)
{
    while (1) {
        ...
        static struct option long_options[] = {
            ...
            {"cli",         optional_argument, 0,  OPT_CLI },
        ...
        case OPT_CLI:
            cli_path = optarg ? optarg : "/var/run/ivs-cli.sock";
            break;
        ...
        case '?':
            ...
            printf("  --cli[=PATH]                Listen for CLI commands on a unix domain socket\n");
...
aim_main(int argc, char* argv[])
{
    ...
    if (cli_path != NULL) {
        AIM_LOG_MSG("Creating CLI socket at %s", cli_path);
        ivs_cli_init(cli_path);
    }
